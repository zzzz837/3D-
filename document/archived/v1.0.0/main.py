...
class MainWindow(QMainWindow):
    ...
    def _new_project(self):
        ...
        path, _ = QFileDialog.getOpenFileName(self, "选择3D模型", "",
            "3D模型 (*.stl *.obj *.glb *.gltf *.stp *.step);;STL (*.stl);;OBJ (*.obj);;GLB/GLTF (*.glb *.gltf);;STEP (*.stp *.step);;All (*.*)")
        if path:
            fmt = Path(path).suffix.lstrip('.').lower()
            self._model_name = Path(path).name
            # STP/STEP: convert to STL via pythonocc-core subprocess
            if fmt in ('stp', 'step'):
                self._sl.setText(f"正在转换STEP格式...")
                try:
                    converter = os.path.join(os.path.dirname(__file__), "stp_convert.py")
                    result = subprocess.run(
                        ["conda", "run", "-n", "occ", "python", converter, path],
                        capture_output=True, timeout=120
                    )
                    if result.returncode != 0:
                        raise RuntimeError(result.stderr.decode('utf-8', errors='replace'))
                    raw = result.stdout
                    fmt = 'stl'  # converted to STL
                    self._model_format = 'stl'
                except Exception as e:
                    QMessageBox.warning(self, "STEP转换失败",
                        f"无法转换STEP文件。请确保已安装conda环境'occ'中的pythonocc-core。\n\n{str(e)}")
                    return
            else:
            ...
            self._model_b64 = b64
            self.bridge.cmd("loadStl", {"b64":b64,"name":Path(path).name,"format":fmt})
    ...
    def _open_profile(self):
        ...
        try:
            ...
            if model_b64:
                ...
                self.bridge.cmd("loadStl", {"b64":model_b64,"name":model_name,"cells":cells,"total_points":total,"format":self._model_format})
